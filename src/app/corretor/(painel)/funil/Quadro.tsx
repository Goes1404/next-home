...
const COR_COLUNA: Record<EtapaFunil, string> = {
  novo: "border-brand-300/40",
  primeiro_contato: "border-brand-400/30",
  visita_agendada: "border-azure-300/30",
  proposta_enviada: "border-sand-400/30",
  negociacao: "border-sand-400/40",
  fechado: "border-brand-300/50",
  perdido: "border-white/10",
};
  ...
  return (
    <div className="mt-6">
      {erro && (
        <p
          role="alert"
          className="text-fluid-sm mb-4 rounded-xl border border-sand-400/40 bg-sand-400/10 px-4 py-3 text-sand-300"
        >
          {erro}
        </p>
      )}
      ...
      <div className="scrollbar-none -mx-4 flex gap-3 overflow-x-auto px-4 pb-4">
        {ETAPAS_FUNIL.map((etapa) => {
          ...
          return (
            <section
              ...
              className={`w-72 shrink-0 rounded-2xl border bg-ink-900/40 p-3 ${COR_COLUNA[etapa]} ${
            ...
            >
              <header className="flex items-baseline justify-between px-1 pb-3">
                ...
                <span className="text-fluid-xs text-mist-500">{daEtapa.length}</span>
              </header>
              ...
              <div className="space-y-2">
                ...
                {daEtapa.length === 0 && (
                  <p className="text-fluid-xs px-1 py-6 text-center text-mist-500">
                    Vazio
                  </p>
                )}
              </div>
    ...
    >
      ...
      <p className="text-fluid-xs mt-1 text-mist-500">
        ...
        {parado !== null && parado >= 3 && (
          <span className="text-sand-300"> · parado há {parado} dias</span>
        )}
      </p>

      {mostrarDono && (
        <p className="text-fluid-xs mt-1 text-mist-400">{lead.corretor?.nome ?? "Sem dono"}</p>
      )}
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        <select
          ...
          className="text-fluid-xs min-w-0 flex-1 rounded-lg border border-white/15 bg-ink-950 px-2 py-1.5 text-mist-200"
        >
          {ETAPAS_FUNIL.map((etapa) => (
        ...
        {whatsapp && (
          <a
            ...
            className="shrink-0 rounded-lg bg-brand-500 p-1.5 text-white transition-colors hover:bg-brand-400"
          >
