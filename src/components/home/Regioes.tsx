  ...
  return (
    <section className="px-4 pb-16 sm:pb-24">
      <div className="mx-auto w-full max-w-3xl text-center">
        <Reveal>
          <h2 className="text-fluid-2xl text-mist-50">Onde atuamos</h2>
          <p className="text-fluid-base mt-3 text-mist-300">
            Da Grande Alphaville até Osasco, sempre com quem conhece a região de perto.
          </p>
        </Reveal>
        ...
        <Reveal stagger={0.08} className="mt-8 flex flex-wrap items-center justify-center gap-3">
          {REGIOES.map((r) => (
            <Link
              key={r.nome}
              href={r.href}
              className="rounded-full border border-white/15 bg-ink-900/50 px-5 py-2.5 text-sm text-mist-100 transition-colors hover:border-brand-300/50 hover:text-brand-200"
            >
              {r.nome}
            </Link>
          ))}
